...
module regfile(input  logic        clk,
    ...
    initial begin
        // Start registers at zero in simulation to avoid X values from unused
        // source fields in instructions like addi/lw.
    ...
    // Register x0 is hardwired to zero, as required by the RISC-V ISA.
...
module alu(input  logic [31:0] a, b,
    ...
    // Reuse the adder for subtraction by adding the two's complement of b.
...
module extend(input  logic [31:7] instr,
    ...
    // Rebuild each immediate format from the non-contiguous instruction fields
    // and sign-extend it to the 32-bit datapath width.
    always_comb
        case (immsrc)
            // I-type
...
module mux3 #(parameter WIDTH = 8)
             (input  logic [WIDTH-1:0] d0, d1, d2,
              input  logic [1:0]       s,
              output logic [WIDTH-1:0] y);
    // Drive X on illegal selects so control bugs are visible in simulation.
    assign y = (s == 2'b00) ? d0 :
               (s == 2'b01) ? d1 :
