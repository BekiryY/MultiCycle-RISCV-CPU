...
module datapath(input  logic        clk, reset,
    ...
    // PC holds the current fetch address; OldPC keeps the fetch PC for branches
    // and JAL while the main PC is advanced during the fetch cycle.
    logic [31:0] PC, OldPC;
    ...
    // Register fields are taken directly from the latched instruction.
    logic [4:0] A1, A2, A3;
    ...
    // Register file reads rs1/rs2 and writes rd during the writeback states.
    ...
    // ALU source muxes let the same ALU update PC, form addresses, compare
    // branch operands, and execute arithmetic instructions.
    ...
    // ALUOut saves the result for the next cycle, which is needed because this
    // multicycle design reuses the ALU across several instruction phases.
