...
module testbench();
    ...
    initial begin
        ...
        // Print first few memory contents to verify riscvtest.txt loaded
        #1;
        $display("=== Memory check (riscvtest.txt load test) ===");
        ...
        if (dut.mem.RAM[0] === 32'h00500113)
            $display(">>> riscvtest.txt LOADED OK");
        else
            $display(">>> ERROR: riscvtest.txt NOT LOADED");
        $display("============================================");
    ...
    // Print core state each cycle for debug
    always @(posedge clk) begin
        if (!reset) begin
            $display("t=%0t  PC=%h  Instr=%h  state=%0d  MemWrite=%b  DataAdr=%h  ReadData=%h  WriteData=%h",
                     $time,
                     dut.riscv.dp.PC,
                     dut.riscv.dp.Instr,
                     dut.riscv.c.fsm.state,
                     MemWrite, DataAdr, dut.ReadData, WriteData);
        end
    end
